    ...
    def run(self):
        ...
        if self.parseduri.name in tmplist:
            ...
            pub.sendMessage('add',
                            args=[
                                self.parseduri.name,
                                datetime.datetime.now(), self._args[3],
                                self._args[4], 'no'
                            ])
        while self.added.status() and not self.deleted:
            ...
            with open('resumedata/' + self.parseduri.name, 'wb') as f:
                f.write(
                    lt.write_resume_data_buf(lt.parse_magnet_uri(
                        self._args[3])))
            ...
            if self.se.total_done >= 1000000000:
                ...
                self.doneunit = 'MB'
            if self.total == 0.0:
                self.total = self.total_done
                self.unit = self.doneunit

            wx.PostEvent(
                self._kwargs,
                ResultEvent([
                    self.se.progress * 100, self.se.num_seeds,
                    self.se.num_peers, self.se.download_rate / 1000000,
                    self.se.upload_rate / 1000000, self.se.state,
                    self.parseduri.name, self.total, self.total_done,
                    self.unit, self.doneunit
                ]))
            time.sleep(3)
...
class magntdialog(wx.Dialog):
    ...
    def getmagnet(self, event):
        pub.sendMessage('addfromdiag',
                        x=[
                            None, None, None,
                            self.entry.GetValue(),
                            self.entry2.GetValue()
                        ])
        magntdialog.Destroy(self)

    def setpath(self, event):
        filepath = wx.DirDialog(None, 'Select Folder', './downloads/',
                                wx.DD_DEFAULT_STYLE)
        filepath.ShowModal()
        filepath.Destroy()
...
class MyFrame(wx.Frame):
    ...
    def __init__(self, *args, **kw):
        ...
        self.db.commit()
        self.cur.execute(
            'CREATE TABLE IF NOT EXISTS downloads (name, date, link, path, paused)'
        )
        self.panel = wx.Panel(self)
        self.boxsizer = wx.BoxSizer(wx.VERTICAL)
        self.indeex = 0
        self.ult = ULC.UltimateListCtrl(self.panel,
                                        agwStyle=ULC.ULC_REPORT
                                        | ULC.ULC_HAS_VARIABLE_ROW_HEIGHT)
        for index, i in enumerate([
                'Name', 'Size', 'Progress', 'Downloaded', 'Status',
                'Down Speed', 'Up Speed', 'Seeds', 'Peers'
        ]):
    ...
    def OnPause(self, event):
        ...
        if self.all[ind][-1] == 'yes':
            self.cur.execute(
                'UPDATE downloads SET paused = :nai WHERE oid =' +
                str(ind + 1), {'nai': 'no'})
        else:
            self.cur.execute(
                'UPDATE downloads SET paused = :nai WHERE oid =' +
    ...
    def OnDelete(self, event):
        ...
        pub.sendMessage(
            'deletetor',
            args=[self.alldowns[self.ind][1], self.alldowns[self.ind][4]])
        self.cur.execute('DELETE FROM downloads WHERE oid =' +
                         str(self.ind + 1))
        self.cur.execute('SELECT oid,* FROM downloads')
        self.alldowns = self.cur.fetchall()
    ...
    def updateprog(self, message):
        ...
        for x, y in enumerate(self.alldowns):
            if y[1] == message.data[6]:
                ...
                self.ult.SetStringItem(
                    x, 3,
                    str(round(message.data[8], 2)) + message.data[10])
    ...
    def addtor(self, args):
        ...
        if args[0] not in tmplist:
            self.cur.execute(
                'INSERT INTO downloads VALUES (:torname,:tordate,:link, :path, :ispaused)',
                {
                    'torname': args[0],
                    'tordate': args[1],
                    'link': args[2],
